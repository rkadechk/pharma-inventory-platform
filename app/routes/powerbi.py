...
async def export_transfer_coordination(
    ...
    try:
        ...
        # Per-category unit costs (mirrors the generator)
        UNIT_COSTS = {
            "Cardiovascular":    18.50, "Endocrine":         12.75,
            "GI":                 8.20, "Antibiotic":        22.40,
            "Pain":               6.80, "Respiratory":       35.00,
            "Psychiatric":       14.60, "Immunosuppressant": 28.90,
            "Antihistamine":      5.40, "Anticoagulant":     42.00,
            "Antiplatelet":       4.90,
        }
        ...
        for _, row in trans_df.iterrows():
            ...
            qty     = float(row.get('quantity_transferred', row.get('quantity_requested', 0)) or 0)
            cost    = float(row.get('cost_usd', 0) or 0)
            category   = cat_dict.get(med_id, "Pain")
            unit_cost  = UNIT_COSTS.get(category, 10.0)
            med_value  = qty * unit_cost
            # Use pre-computed values from CSV if available, otherwise derive
            est_savings  = float(row['estimated_savings']  if 'estimated_savings'  in row.index and pd.notna(row['estimated_savings'])  else med_value * 0.30)
            eff_score    = float(row['efficiency_score']   if 'efficiency_score'   in row.index and pd.notna(row['efficiency_score'])   else min(1.0, est_savings / max(cost + med_value * 0.05, 1)))
            cb_score     = float(row['cost_benefit_score'] if 'cost_benefit_score' in row.index and pd.notna(row['cost_benefit_score']) else min(1.5, est_savings / max(cost, 1)))
            transfer_stat = str(row.get('transfer_status', 'PENDING'))
            compliance    = "REVIEW" if transfer_stat == "BLOCKED" else "OK"
            ...
            records.append({
                ...
                "transfer_cost_per_unit": round(cost / max(qty, 1), 3),
                "total_transfer_cost":    round(cost, 2),
                "total_medication_value": round(med_value, 2),
                "decision_type":          "TRANSFER",
                "status":                 transfer_stat,
                "compliance_status":      compliance,
                "expected_delivery_date": (str(row['received_date'])
                                             if 'received_date' in row.index and pd.notna(row.get('received_date'))
                                             else str(row.get('transfer_date', ''))),
                ...
                "cost_benefit_score":     round(min(cb_score, 1.5), 3),
            })
    ...
    try:
        ...
        for _, row in forecast_df.iterrows():
            ...
            records.append({
                ...
                "suggested_action": str(row.get('suggested_action', 'MONITOR') or 'MONITOR'),
                "urgency": str(row.get('urgency_level', 'MEDIUM') or 'MEDIUM'),
                "risk_level": str(row.get('risk_level', 'LOW') or 'LOW'),
                "anomalies_detected": int(row.get('anomalies_detected', 0) or 0),
                "external_signals": (str(row['external_signals'])
                                      if 'external_signals' in row.index
                                         and pd.notna(row['external_signals'])
                                         and str(row['external_signals']) not in ('nan', 'None', '')
                                      else 'None'),
            })
...
async def get_available_date_range():
    """
    Get the min and max dates available in the SQLite database
    Used by Streamlit calendar to constrain date picker
    Returns dates for: expiry_date, created_date, forecast_date, expected_delivery_date
    """
    try:
        inventory_df, _, _, trans_df, forecast_df = load_all_data()
        ...
        if not inventory_df.empty and 'expiration_date' in inventory_df.columns:
            try:
                inv_dates = pd.to_datetime(inventory_df['expiration_date'], errors='coerce')
                inv_dates = inv_dates.dropna()
                if len(inv_dates) > 0:
                    date_ranges['expiry_date'] = {
                        'min': inv_dates.min().strftime('%Y-%m-%d'),
                        'max': inv_dates.max().strftime('%Y-%m-%d'),
                    }
            except Exception as e:
                logger.warning(f"Error parsing inventory dates: {e}")
        
        # Transfer dates (created_date and expected_delivery_date)
        if not trans_df.empty:
            try:
                if 'created_date' in trans_df.columns:
                    created_dates = pd.to_datetime(trans_df['created_date'], errors='coerce')
                    created_dates = created_dates.dropna()
                    if len(created_dates) > 0:
                        if 'created_date' not in date_ranges:
                            date_ranges['created_date'] = {}
                        date_ranges['created_date']['min'] = created_dates.min().strftime('%Y-%m-%d')
                        date_ranges['created_date']['max'] = created_dates.max().strftime('%Y-%m-%d')
                
                if 'received_date' in trans_df.columns or 'transfer_date' in trans_df.columns:
                    delivery_col = 'received_date' if 'received_date' in trans_df.columns else 'transfer_date'
                    delivery_dates = pd.to_datetime(trans_df[delivery_col], errors='coerce')
                    delivery_dates = delivery_dates.dropna()
                    if len(delivery_dates) > 0:
                        date_ranges['expected_delivery_date'] = {
                            'min': delivery_dates.min().strftime('%Y-%m-%d'),
                            'max': delivery_dates.max().strftime('%Y-%m-%d'),
                        }
            except Exception as e:
                logger.warning(f"Error parsing transfer dates: {e}")
        
        # Forecast dates
        if not forecast_df.empty and 'forecast_date' in forecast_df.columns:
            try:
                forecast_dates = pd.to_datetime(forecast_df['forecast_date'], errors='coerce')
                forecast_dates = forecast_dates.dropna()
                if len(forecast_dates) > 0:
                    date_ranges['forecast_date'] = {
                        'min': forecast_dates.min().strftime('%Y-%m-%d'),
                        'max': forecast_dates.max().strftime('%Y-%m-%d'),
                    }
            except Exception as e:
                logger.warning(f"Error parsing forecast dates: {e}")
        
        # Calculate overall range (across all date types)
        all_min_dates = []
        all_max_dates = []
        for date_type, date_range in date_ranges.items():
            if 'min' in date_range:
                all_min_dates.append(datetime.strptime(date_range['min'], '%Y-%m-%d'))
            if 'max' in date_range:
                all_max_dates.append(datetime.strptime(date_range['max'], '%Y-%m-%d'))
        
        overall_min = min(all_min_dates).strftime('%Y-%m-%d') if all_min_dates else None
        overall_max = max(all_max_dates).strftime('%Y-%m-%d') if all_max_dates else None
        ...
        # Return fallback with last 90 days
        today = datetime.now()
        ninety_days_ago = today - timedelta(days=90)
        return {
            "available_dates": {
                "created_date": {
                    "min": ninety_days_ago.strftime('%Y-%m-%d'),
                ...
                "expiry_date": {
                    ...
                    "max": (today + timedelta(days=365)).strftime('%Y-%m-%d'),
                }
            },
            "overall_range": {
                "min_date": ninety_days_ago.strftime('%Y-%m-%d'),
